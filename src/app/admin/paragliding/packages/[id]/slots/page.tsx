...
function SlotEditCell({ slot, onSaved }: { slot: Slot; onSaved: () => void }) {
  ...
  function save() {
    ...
    startTransition(async () => {
      const res = await fetch(`/api/admin/paragliding/slots/${slot.id}`, {
        method: "PATCH",
...
export default function AdminPackageSlotsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  function handleCreate(e: React.FormEvent) {
    ...
    setError(null);
    startTransition(async () => {
      const res = await fetch(`/api/admin/paragliding/packages/${id}/slots`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setForm({ date: "", startTime: "07:00", capacity: "4" });
      load();
    });
  }
  ...
  function handleDelete(slotId: string) {
    startTransition(async () => {
      await fetch(`/api/admin/paragliding/slots/${slotId}`, { method: "DELETE" });
      load();
    });
  }
  ...
  return (
    <div>
      <Link href="/admin/paragliding/packages" className="text-sm text-muted hover:text-ink">
        ← Back to packages
      </Link>

...
export function BookTravelWidget({
  routeId,
  slots,
  unitPrice,
  isLoggedIn,
  ...
  const total = useMemo(() => unitPrice * passengers, [unitPrice, passengers]);
  ...
  function handleBook() {
    ...
    startTransition(async () => {
      const res = await fetch("/api/bookings", {
        ...
        body: JSON.stringify({
          items: [{ itemType: "TRAVEL", routeId, slotId, passengers }],
  ...
  if (slots.length === 0) {
    return <p className="text-sm text-muted">No upcoming departures — check back soon.</p>;
  }
          ...
          {slots.map((slot) => {
            ...
            return (
              <option key={slot.id} value={slot.id} disabled={left <= 0}>
                {formatDate(slot.date)} · {slot.departureTime} · {left > 0 ? `${left} seats left` : "Full"}
              </option>
            );
          })}
        </select>
